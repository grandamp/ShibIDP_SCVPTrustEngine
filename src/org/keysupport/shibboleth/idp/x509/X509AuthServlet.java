...
/**
 * Servlet compatible with the {@link ExternalAuthentication} interface that
 * extracts and validates an X.509 client certificate for user authentication.
 */
public class X509AuthServlet extends HttpServlet {
	 ...
	 *            trust engine to use
	 */
	public void setTrustEngine(
			@Nullable final TrustEngine<? super X509Credential> tm) {
		trustEngine = tm;
	}
	...
	public void init(final ServletConfig config) throws ServletException {
		...
		if (param != null) {
			...
			if (bean instanceof TrustEngine) {
				...
				throw new ServletException("Bean " + param
						+ " was missing, or not a TrustManager");
			}
		}
	}
	...
	protected void service(final HttpServletRequest httpRequest,
		...
		try {
			...
			log.debug(
					"End-entity X.509 certificate found with subject '{}', issued by '{}'",
			...
			final String passthrough = httpRequest
					.getParameter(PASSTHROUGH_PARAM);
